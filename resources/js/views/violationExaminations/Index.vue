            ...
            <div class="intro-y col-span-12 lg:col-span-12">
                ...
                <div class="intro-y box">
                    <div class="flex flex-col sm:flex-row items-center p-5 border-b border-gray-200 dark:border-dark-5">
                        <h2 class="font-medium text-base mr-auto">Violation Examination</h2>

                        <div class="form-check w-full sm:w-auto sm:ml-auto mt-3 sm:mt-0">
                            <button class="btn btn-primary shadow-md mr-2" @click.prevent="$router.push({name: 'admin.violation-examinations.create'})">
                    ...
                    <div class="p-5">
                        <div class="preview">
                            ...
                            <div class="grid grid-cols-12 gap-6">
                                ...
                                <div class="intro-y col-span-12 overflow-auto">
                                    <BaseTableComponent :columns="columns" :childColumns="childColumns" :sortKey="sortKey" :sortOrders="sortOrders" @sortBy="sortBy">
                                        ...
                                        <tr class="hover:bg-gray-200" v-for="row in getCollection.data" :key="row.id">
                                            <td class="border-b dark:border-dark-5 text-center">{{ row.violation_website | numFormat }}</td>
                                            <td class="border-b dark:border-dark-5 text-center">{{ row.violation_newspaper | numFormat  }}</td>
                                            <td class="border-b dark:border-dark-5 text-center">{{ row.violation_identified | numFormat  }}</td>
                                            <td class="border-b dark:border-dark-5 text-center">{{ row.violation_removed | numFormat  }}</td>
                                            <td class="border-b dark:border-dark-5 text-center">{{ row.violation_pending | numFormat  }}</td>
                                            <td class="border-b dark:border-dark-5 text-center">{{ row.created_at  }}</td>
                                            <td class="border-b dark:border-dark-5 text-center"><span :class="row.violation_status?'text-theme-9':'text-theme-6'">{{ row.violation_status?'Active':'Inactive' }}</span></td>
                                            <td class="border-b dark:border-dark-5 text-center">
                                                <router-link :to="{ name: 'admin.violation-examinations.edit', params: { id: row.id } }">
                                ...
                                <p class="intro-y col-span-12 flex flex-wrap sm:flex-row sm:flex-nowrap items-center" v-if="getCollection.meta && !getCollection.meta.total">Currently violation examination is added.</p>
...
export default {
    name: "ViolationExamination",
    data(){
        let sortOrders = {};

        let columns = [
            { label: 'No. of Documents Examined for Violations', name: null, colspan:2 },
            { label: 'No. of Violations Identified', name: 'case_disposed_off', orderable: true},
            { label: 'No. of Violations Removed', name: 'case_disposed_off', orderable: true},
            { label: 'No. of Pending Violations Department Wise', name: 'case_disposed_off', orderable: true},
            { label: 'Date', name: 'created_at', orderable: true},
            { label: 'Status', name: 'violation_status', orderable: true},
        ...
        let childColumns = [
            { label: 'Website', name: 'violation_website', orderable: true },
            { label: 'Newspaper', name: 'violation_newspaper', orderable: true },
            { label: '', name: null },
            { label: '', name: null },
            { label: '', name: null },
            { label: '', name: null },
    ...
    methods: {
        loadCollection: function(url = '/api/v1/violation-examinations'){
        ...
        deleteRecord: function (delete_id = this.delete_id){
            this.processing = true;
            axios.delete(`/api/v1/violation-examinations/${delete_id}`).then((resp) => {
