            ...
            <div class="intro-y col-span-12 lg:col-span-12">
                ...
                <div class="intro-y box">
                    <div class="flex flex-col sm:flex-row items-center p-5 border-b border-gray-200 dark:border-dark-5">
                        <h2 class="font-medium text-base mr-auto">Edit Trainings</h2>
                    </div>
                    <div class="p-5">
                        <div class="preview">
                            ...
                            <form class="validate-form w-1/2">
                                <div class="input-form" :class="{'has-error':(errors && errors.case_initiated)}">
                                    ...
                                    <div class="pristine-error text-primary-3 mt-2" v-if="errors && errors.training_persons_imparted">
                                        {{  errors.training_persons_imparted[0] }}</div>
                                </div>


                                <div class="input-form mt-5" :class="{'has-error':(errors && errors.training_status)}">
                                    <label class="form-label w-full flex flex-col sm:flex-row">
                                    ...
                                    <div class="flex flex-col sm:flex-row mt-2">
                                        <div class="form-check mr-2">
                                            <input v-model="form.training_status" id="active_status" class="form-check-input" type="radio" name="training_status" value="1">
                                            <label class="form-check-label" for="active_status">Active</label>
                                        </div>
                                        <div class="form-check mr-2 mt-2 sm:mt-0">
                                            <input v-model="form.training_status" id="inactive_status" class="form-check-input" type="radio" name="training_status" value="0">
                                            <label class="form-check-label" for="inactive_status">Inactive</label>
                                        </div>
                                    </div>

                                    <div class="pristine-error text-primary-3 mt-2" v-if="errors && errors.training_status">
                                        {{  errors.training_status[0] }}</div>
                                </div>
                                ...
                                <button type="button" class="btn btn-secondary mt-5" @click.prevent="$router.push({name: 'admin.trainings.index'})">Cancel</button>
                            </form>
                            <!-- END: Validation Form -->
...
export default {
    ...
    methods: {
        loadData: function (){
            axios.get('/api/v1/trainings/' + this.$route.params.id).then(response => {
                this.form = response.data.row;
            }).catch((error)=>{
                this.processing = false
                this.$vToastify.error("Record has been not found.");
                this.$router.push({name: 'admin.trainings.index'});
            });
        },
        updateForm: function (){
            this.processing = true;
            axios.put(`/api/v1/trainings/${this.$route.params.id}`, this.form).then((resp) => {
                this.processing = false
                if(resp.data.error){
