            ...
            <div class="intro-y col-span-12 lg:col-span-12">
                ...
                <div class="intro-y box">
                    <div class="flex flex-col sm:flex-row items-center p-5 border-b border-gray-200 dark:border-dark-5">
                        <h2 class="font-medium text-base mr-auto">Edit Attribute Value</h2>
                    </div>
                    <div class="p-5">
                        <div class="preview">
                            ...
                            <form class="validate-form lg:w-1/2">
                                <div class="input-form" :class="{'has-error':(errors && errors.value)}">
                                    <label class="form-label w-full flex flex-col sm:flex-row">
                                        Attribute Value<span class="text-primary-3">*</span>
                                    </label>
                                    <input type="text" placeholder="Name"  class="form-control" v-model="form.value" required />
                                    <div class="pristine-error text-primary-3 mt-2" v-if="errors && errors.value">
                                        {{  errors.value[0] }}</div>
                                </div>
                                ...
                                <button type="button" class="btn btn-secondary mt-5" @click.prevent="$router.push({name: 'admin.attributes.attribute-values.index', params: { attribute_id: $route.params.attribute_id }})">Cancel</button>
                            </form>
                            <!-- END: Validation Form -->
...
export default {
    ...
    methods: {
        loadData: function (){
            axios.get(`/api/v1/attributes/${this.$route.params.attribute_id}/attribute-values/${this.$route.params.id}`).then(response => {
                this.form = response.data.row;
                this.form['_method'] = 'PUT';
            }).catch((error)=>{
                this.processing = false
                this.$vToastify.error("Record has been not found.");
                this.$router.push({name: 'admin.attributes.attribute-values.index', params: { attribute_id: this.$route.params.attribute_id } });
            });
        },
        updateForm: function (){
            this.processing = true;
            axios.post(`/api/v1/attributes/${this.$route.params.attribute_id}/attribute-values/${this.$route.params.id}`, this.form).then((resp) => {
                this.processing = false
                if(resp.data.error){
