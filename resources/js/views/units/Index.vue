            ...
            <div class="intro-y col-span-12 lg:col-span-12">
                ...
                <div class="intro-y box">
                    <div class="flex flex-col sm:flex-row items-center p-5 border-b border-gray-200 dark:border-dark-5">
                        <h2 class="font-medium text-base mr-auto">Units</h2>

                        <div class="form-check w-full sm:w-auto sm:ml-auto mt-3 sm:mt-0">
                            <button class="btn btn-success shadow-md mr-2" @click.prevent="$router.push({name: 'admin.units.create'})">
                                ...
                                <div class="intro-y col-span-12 overflow-auto lg:overflow-visible">
                                    <BaseTableComponent :columns="columns"  :sortKey="sortKey" :sortOrders="sortOrders" @sortBy="sortBy">
                                        ...
                                        <tr class="hover:bg-gray-200" v-for="row in getCollection.data" :key="row.id">
                                            <td class="border-b dark:border-dark-5 text-center">{{ row.unit_name }}</td>
                                            <td class="border-b dark:border-dark-5 text-center">{{ row.short_name }}</td>
...
export default {
    ...
    data(){
        ...
        let columns = [
            { label: 'Name', name: 'unit_name', orderable: true },
            { label: 'Short Name', name: 'short_name', orderable: true },
            { label: 'Date', name: 'created_at', orderable: true},
            { label: 'Status', name: 'unit_status', orderable: true},
    ...
    methods: {
        loadCollection: function(url = '/api/v1/units'){
        ...
        deleteRecord: function (delete_id = this.delete_id){
            this.processing = true;
            axios.post(`/api/v1/units/${delete_id}`, { _method : 'DELETE' }).then((resp) => {
