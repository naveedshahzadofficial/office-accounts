            ...
            <div class="intro-y col-span-12 lg:col-span-12">
                ...
                <div class="intro-y box">
                    <div class="flex flex-col sm:flex-row items-center p-5 border-b border-gray-200 dark:border-dark-5">
                        <h2 class="font-medium text-base mr-auto">Blacklisting</h2>

                        <div class="form-check w-full sm:w-auto sm:ml-auto mt-3 sm:mt-0">
                            <button class="btn btn-primary shadow-md mr-2" @click.prevent="$router.push({name: 'admin.blacklistings.create'})">
                    ...
                    <div class="p-5">
                        <div class="preview">
                            ...
                            <div class="grid grid-cols-12 gap-6">
                                ...
                                <div class="intro-y col-span-12 overflow-auto ">
                                    <BaseTableComponent :columns="columns" :childColumns="childColumns" :sortKey="sortKey" :sortOrders="sortOrders" @sortBy="sortBy">
                                        ...
                                        <tr class="hover:bg-gray-200" v-for="row in getCollection.data" :key="row.id">
                                            <td class="border-b dark:border-dark-5 text-center">{{ row.blacklist_order | numFormat }}</td>
                                            <td class="border-b dark:border-dark-5 text-center">{{ row.blacklisting_case_disposed_off | numFormat  }}</td>
                                            <td class="border-b dark:border-dark-5 text-center">{{ row.blacklisting_pending_15_to_30 | numFormat  }}</td>
                                            <td class="border-b dark:border-dark-5 text-center">{{ row.blacklisting_pending_more_than_30 | numFormat  }}</td>
                                            <td class="border-b dark:border-dark-5 text-center">{{ row.created_at  }}</td>
                                            <td class="border-b dark:border-dark-5 text-center"><span :class="row.blacklisting_status?'text-theme-9':'text-theme-6'">{{ row.blacklisting_status?'Active':'Inactive' }}</span></td>
                                            <td class="border-b dark:border-dark-5 text-center">
                                                <router-link :to="{ name: 'admin.blacklistings.edit', params: { id: row.id } }">
                                ...
                                <p class="intro-y col-span-12 flex flex-wrap sm:flex-row sm:flex-nowrap items-center" v-if="getCollection.meta && !getCollection.meta.total">Currently no blacklisting is added.</p>
...
export default {
    ...
    data(){
        ...
        let columns = [
            { label: 'Uploading of Blacklisting Order', name: 'blacklist_order', orderable: true },
            { label: 'Blacklisting cases Disposed Off', name: 'blacklisting_case_disposed_off', orderable: true},
            { label: 'Pending Blacklisting Cases', name: null, colspan:2},
        ...
        let childColumns = [
            { label: '', name: null },
            { label: '', name: null },
            { label: 'From 15 to 30 Days', name: 'blacklisting_pending_15_to_30', orderable: true },
            { label: 'More than 30 Days', name: 'blacklisting_pending_more_than_30', orderable: true },
            { label: '', name: null },
            { label: '', name: null },
            { label: '', name: null },
        ];
    ...
    methods: {
        loadCollection: function(url = '/api/v1/blacklistings'){
