                ...
                <div class="intro-y box">
                    <div class="flex flex-col sm:flex-row items-center p-5 border-b border-gray-200 dark:border-dark-5">
                        ...
                        <div class="form-check w-full sm:w-auto sm:ml-auto mt-3 sm:mt-0">
                            <button class="btn btn-success shadow-md mr-2" @click.prevent="$router.push({name: 'admin.users.create'})">
                    ...
                    <div class="p-5">
                        <div class="preview">
                            ...
                            <div class="grid grid-cols-12 gap-6">
                                ...
                                <div class="intro-y col-span-12 overflow-auto lg:overflow-visible">
                                    <BaseTableComponent :columns="columns"  :sortKey="sortKey" :sortOrders="sortOrders" @sortBy="sortBy">
                                        ...
                                        <tr class="hover:bg-gray-200" v-for="row in getCollection.data" :key="row.id">
                                            <td class="border-b dark:border-dark-5 text-center">{{ row.name }}</td>
                                            <td class="border-b dark:border-dark-5 text-center">{{ row.username }}</td>
                                            <td class="border-b dark:border-dark-5 text-center">{{ row.role?row.role.role_name:'' }}</td>
                                            <td class="border-b dark:border-dark-5 text-center">{{ row.created_at  }}</td>
                                            <td class="border-b dark:border-dark-5 text-center">
                                                <div class="flex items-center justify-center text-theme-6" :class="row.user_status?'text-theme-9':'text-theme-6'">{{ row.user_status?'Active':'Inactive'}}</div>
                                            </td>
                                            <td class="border-b dark:border-dark-5 text-center">
                                                <router-link class="mr-2" title="Edit" :to="{ name: 'admin.users.edit', params: { id: row.id } }">
                                ...
                                <p class="intro-y col-span-12 flex flex-wrap sm:flex-row sm:flex-nowrap items-center" v-if="getCollection.meta && !getCollection.meta.total">Currently no user is added.</p>
...
export default {
    ...
    data(){
        ...
        let columns = [
            ...
            { label: 'Username', name: 'username', orderable: true },
            { label: 'Role', name: 'role_id', orderable: true },
            { label: 'Date', name: 'created_at', orderable: true},
            { label: 'Status', name: 'user_status', orderable: true},
    ...
    methods: {
        loadCollection: function(url = '/api/v1/users'){
        ...
        deleteRecord: function (delete_id = this.delete_id){
            this.processing = true;
            axios.post(`/api/v1/users/${delete_id}`, { _method : 'DELETE' }).then((resp) => {
