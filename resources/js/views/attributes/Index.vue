            ...
            <div class="intro-y col-span-12 lg:col-span-12">
                ...
                <div class="intro-y box">
                    <div class="flex flex-col sm:flex-row items-center p-5 border-b border-gray-200 dark:border-dark-5">
                        <h2 class="font-medium text-base mr-auto">Attributes</h2>

                        <div class="form-check w-full sm:w-auto sm:ml-auto mt-3 sm:mt-0">
                            <button class="btn btn-success shadow-md mr-2" @click.prevent="$router.push({name: 'admin.attributes.create'})">
                    ...
                    <div class="p-5">
                        <div class="preview">
                            ...
                            <div class="grid grid-cols-12 gap-6">
                                    ...
                                    <BaseTableComponent :columns="columns"  :sortKey="sortKey" :sortOrders="sortOrders" @sortBy="sortBy">
                                        ...
                                        <tr class="hover:bg-gray-200" v-for="row in getCollection.data" :key="row.id">
                                            ...
                                            <td class="border-b dark:border-dark-5 text-center">
                                                <div class="flex items-center justify-center text-theme-6" :class="row.status?'text-theme-9':'text-theme-6'">{{ row.status?'Active':'Inactive'}}</div>
                                            </td>
                                            <td class="border-b dark:border-dark-5 text-center">

                                                <router-link class="mr-2" title="Attribute Values" :to="{ name: 'admin.attributes.attribute-values.index', params: { attribute_id: row.id } }">
                                                    <feather type="settings" class="text-theme-11 w-4 h-4 mr-1"></feather>
                                                </router-link>

                                                <router-link class="mr-2" title="Edit" :to="{ name: 'admin.attributes.edit', params: { id: row.id } }">
                                ...
                                <p class="intro-y col-span-12 flex flex-wrap sm:flex-row sm:flex-nowrap items-center" v-if="getCollection.meta && !getCollection.meta.total">Currently no attribute is added.</p>
...
export default {
    ...
    methods: {
        loadCollection: function(url = '/api/v1/attributes'){
        ...
        deleteRecord: function (delete_id = this.delete_id){
            this.processing = true;
            axios.post(`/api/v1/attributes/${delete_id}`, { _method : 'DELETE' }).then((resp) => {
