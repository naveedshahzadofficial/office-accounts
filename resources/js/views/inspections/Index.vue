                    ...
                    <div class="p-5">
                        <div class="preview">
                            ...
                            <div class="grid grid-cols-12 gap-6">
                                ...
                                <div class="intro-y col-span-12 overflow-auto">
                                    <BaseTableComponent :columns="columns" :childColumns="childColumns" :sortKey="sortKey" :sortOrders="sortOrders" @sortBy="sortBy">
                                        ...
                                        <tr class="hover:bg-gray-200" v-for="row in getCollection.data" :key="row.id">
                                            <td class="border-b dark:border-dark-5 text-center">{{ row.inspection_internal_conducted | numFormat }}</td>
                                            <td class="border-b dark:border-dark-5 text-center">{{ row.inspection_verification_entries | numFormat  }}</td>
                                            <td class="border-b dark:border-dark-5 text-center">{{ row.inspection_pending_3_to_7 | numFormat  }}</td>
                                ...
                                <p class="intro-y col-span-12 flex flex-wrap sm:flex-row sm:flex-nowrap items-center" v-if="getCollection.meta && !getCollection.meta.total">Currently no inspection is added.</p>
...
export default {
    ...
    methods: {
        loadCollection: function(url = '/api/v1/inspections'){
        ...
        deleteRecord: function (delete_id = this.delete_id){
            this.processing = true;
            axios.post(`/api/v1/inspections/${delete_id}`, { _method : 'DELETE' }).then((resp) => {
