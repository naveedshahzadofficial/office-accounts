                                ...
                                <div class="input-form mt-5" :class="{'has-error':(errors && errors.case_status)}">
                                    ...
                                    <div class="flex flex-col sm:flex-row mt-2">
                                        <div class="form-check mr-2">
                                            <input v-model="form.case_status" id="active_status" class="form-check-input" type="radio" name="case_status" value="1">
                                            <label class="form-check-label" for="active_status">Active</label>
                                        </div>
                                        <div class="form-check mr-2 mt-2 sm:mt-0">
                                            <input v-model="form.case_status" id="inactive_status" class="form-check-input" type="radio" name="case_status" value="0">
                                            <label class="form-check-label" for="inactive_status">Inactive</label>
                                        </div>
                                    </div>

                                    <div class="pristine-error text-primary-3 mt-2" v-if="errors && errors.case_status">
                                        {{  errors.case_status[0] }}</div>
                                </div>
...
export default {
    ...
    methods: {
        loadData: function (){
            axios.get('/api/v1/court-cases/' + this.$route.params.id).then(response => {
        ...
        updateForm: function (){
            ...
            axios.put(`/api/v1/court-cases/${this.$route.params.id}`, this.form).then((resp) => {
                this.processing = false
                if(resp.data.error){
